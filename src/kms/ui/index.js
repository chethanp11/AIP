...
let currentUserClearance = "Internal";
let allCandidates = [];
...
window.actOnCandidate = async function(status) {
    ...
    try {
        ...
        if (res.ok) {
            alert(`Candidate review state set to '${status}' successfully!`);
            if (status === 'Published') {
                alert(`SUCCESS: Approved candidate explicitly published to active SQLite Vector & Graph databases on disk!`);
            }
            closeCandidateModal();
            loadCandidatesReviewQueue();
        } else {
